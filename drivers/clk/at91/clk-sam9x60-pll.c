 ...
 *
 *  Based on clk-sam9x60-pll.c on Linux.
 *
 */
...
static long sam9x60_frac_pll_compute_mul_frac(struct sam9x60_pll *pll,
					      ...
					      ulong parent_rate)
{
	...
	if (tmprate < characteristics->output[0].min &&
	    tmprate > characteristics->output[0].max)
		return -ERANGE;
...
static ulong sam9x60_frac_pll_set_rate(struct clk *clk, ulong rate)
{
	...
	ret = sam9x60_frac_pll_compute_mul_frac(pll, &nmul, &nfrac, rate,
						parent_rate);
	if (ret < 0)
		return 0;
...
static int sam9x60_frac_pll_enable(struct clk *clk)
{
	...
	if (crate < characteristics->output[0].min ||
	    crate > characteristics->output[0].max)
		return -ERANGE;
