...
{
	struct clk_main_rc *main_rc = to_clk_main_rc(dev_get_clk_ptr(clk->dev));
...
static int clk_main_osc_enable(struct clk *clk)
{
	...
	if (val & (AT91_PMC_OSCBYPASS | AT91_PMC_MOSCEN))
		return 0;
			      ...
			      const char *parent_name, bool bypass)
{
	...
	if (bypass) {
		pmc_update_bits(reg, AT91_CKGR_MOR, MOR_KEY_MASK |
				AT91_PMC_OSCBYPASS,
