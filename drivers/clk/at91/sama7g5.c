...
static const struct pmc_clk_setup {
	...
	/* PLL clocks. */
	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_CPU_FRAC),
		.rate = 800000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_SYS_FRAC),
		.rate = 1200000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_SYS_DIV),
		.rate = 400000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_DDR_FRAC),
		.rate = 1066000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_DDR_DIV),
		.rate = 533000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_IMG_FRAC),
		.rate = 1064000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_IMG_DIV),
		.rate = 266000000,
	},

	/* MCK clocks. */
	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MCK0),
		.pid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_CPU_DIV),
		.rate = 200000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MCK1),
		.pid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_SYS_DIV),
		.rate = 200000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MCK2),
		.pid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_DDR_DIV),
		.rate = 533000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MCK3),
		.pid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_IMG_DIV),
		.rate = 266000000,
	},

	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MCK4),
		.pid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_PLL_SYS_DIV),
		.rate = 400000000,
	},
	...
	/* Generic clocks. */
	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_GCK, 81),
...
/* Clock cleanups to be applied due to previous bootloaders. */
static const struct pmc_clk_setup sama7g5_restore_clk_setup[] = {
#ifdef CONFIG_SPL_BUILD
	{
		.cid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MCK4),
		.pid = AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_TD_SLCK),
		.rate = 32768,
	},
...
static int sama7g5_clk_probe(struct udevice *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(sama7g5_mckx); i++) {
		...
		clk_dm(AT91_TO_CLK_ID(PMC_TYPE_CORE, sama7g5_mckx[i].cid),
			...
			tmpmux, tmpclkmux, sama7g5_mckx[i].f,
			sama7g5_mckx[i].id));
	}
	...
	/* Clock restore due to previous bootloader setups. */
	/* TODO: test this! */
	for (i = 0; i < ARRAY_SIZE(sama7g5_restore_clk_setup); i++) {
		ret = clk_get_by_id(sama7g5_restore_clk_setup[i].cid, &c);
		if (ret)
			goto fail;

		if (sama7g5_restore_clk_setup[i].pid) {
			ret = clk_get_by_id(sama7g5_restore_clk_setup[i].pid,
					    &parent);
			if (ret)
				goto fail;
...
static void sama7g5_dump_clk(int id, struct udevice *dev)
{
	struct clk *clk, *parent;
	long ret;

	ret = clk_get_by_id(id, &clk);
	if (ret) {
		debug("%s(): clk_request() failed: %ld\n", __func__, ret);
		return;
	}

	ret = clk_get_rate(clk);
	if (ret < 0) {
		debug("%s(): clk_get_rate() failed: %ld\n", __func__, ret);
		return;
	}

	parent = clk_get_parent(clk);
	printf("%-20s%-10d%-15lu%-20s\n", clk_hw_get_name(clk), id, (ulong)ret,
		IS_ERR(parent) ? "NA" : clk_hw_get_name(parent));
}
...
int soc_clk_dump(void)
{
	...
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_sckc), &dev);
	if (ret)
		return ret;
	for (i = 0; i < 2; i++)
		sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_SLOW, i), dev);

	/* Main RC. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_main_rc_clk), &dev);
	if (ret)
		return ret;
	sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MAIN_RC_OSC), dev);

	/* Main Osc. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_main_osc_clk), &dev);
	if (ret)
		return ret;
	sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MAIN_OSC), dev);

	/* Mainck. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_sam9x5_main_clk), &dev);
	if (ret)
		return ret;
	sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MAINCK), dev);

	/* Frac PLLs. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_sam9x60_frac_pll_clk), &dev);
	if (ret)
		return ret;
	for (i = 0; i < ARRAY_SIZE(sama7g5_plls); i++)
		if (sama7g5_plls[i].t == PLL_TYPE_FRAC)
			sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_CORE,
					 sama7g5_plls[i].cid), dev);

	/* Div PLLs. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_sam9x60_div_pll_clk), &dev);
	if (ret)
		return ret;
	for (i = 0; i < ARRAY_SIZE(sama7g5_plls); i++)
		if (sama7g5_plls[i].t == PLL_TYPE_DIV)
	...
	/* MCK0. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_master_clk), &dev);
	if (ret)
		return ret;
	sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_MCK0), dev);

	/* MCK1..4. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_sama7g5_master_clk), &dev);
	if (ret)
		return ret;
	for (i = 0; i < ARRAY_SIZE(sama7g5_mckx); i++)
		sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_CORE,
			sama7g5_mckx[i].cid), dev);

	/* UTMI. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_sama7g5_utmi_clk), &dev);
	if (ret)
		return ret;
	sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_CORE, ID_UTMI), dev);

	/* Programmable clocks. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_prog_clk), &dev);
	if (ret)
		return ret;
	for (i = 0; i < ARRAY_SIZE(sama7g5_prog); i++)
		sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_CORE,
			sama7g5_prog[i].cid), dev);

	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_system_clk), &dev);
	if (ret)
		return ret;
	for (i = 0; i < ARRAY_SIZE(sama7g5_systemck); i++)
		sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_SYSTEM,
			sama7g5_systemck[i].cid), dev);

	/* Peripheral clocks. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_sam9x5_periph_clk), &dev);
	if (ret)
		return ret;
	for (i = 0; i < ARRAY_SIZE(sama7g5_periphck); i++)
		sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_PERIPHERAL,
			sama7g5_periphck[i].id), dev);

	/* Generic clocks. */
	ret = uclass_get_device_by_driver(UCLASS_CLK,
		DM_GET_DRIVER(at91_gck_clk), &dev);
	if (ret)
		return ret;
	for (i = 0; i < ARRAY_SIZE(sama7g5_gck); i++)
		sama7g5_dump_clk(AT91_TO_CLK_ID(PMC_TYPE_GCK,
			sama7g5_gck[i].id), dev);
