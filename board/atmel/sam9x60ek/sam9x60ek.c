...
static void at91sam9x5ek_nand_hw_init(void)
{
	...
	csa |= AT91_SFR_EBI_CS3A_SMC_SMARTMEDIA;
	/* NAND flash on D16 */
	csa |= AT91_SFR_NFD0_ON_D16;

	/* Configure IO drive */
	csa &= ~AT91_SFR_EBI_EBI_IOSR_NORMAL;
...
void mem_init(void)
{
	...
	csa = readl(&sfr->ebicsa);
	csa |= AT91_SFR_EBI_CS1A_SDRAMC;
	csa &= ~AT91_SFR_EBI_DBPU_OFF;
	csa |= AT91_SFR_EBI_DBPD_OFF;
	csa |= AT91_SFR_EBI_EBI_IOSR_NORMAL;
	writel(csa, &sfr->ebicsa);
